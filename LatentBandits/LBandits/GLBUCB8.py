    ...
    def choose_Col_RR(self,user):
        ...
        while True:
            ...
            if self.remExplore() > 0:
                if chooseCol < self.numActions and self.B[chooseCol] != -1 and self.expl[chooseCol]!=-1:
                    return chooseCol
                else:
                    if chooseCol < self.numActions:
                        chooseCol = chooseCol + 1
            else:
                #self.write_file(self.t,self.estR,'_R_')
                chooseCol = random.randint(0,self.numActions-1)
                #print self.expl
                if self.B[chooseCol] != -1:
                    return chooseCol
        ...
        for user in range(0,self.users):
            for col in range(0,self.users):
                ...
                for col1 in range(0,self.users):
                    #if col!= col1 and temp[user][col] > temp[user][col1] and temp[user][col]!=self.MIN and temp[user][col1]!=self.MIN:
                    if col!= col1 and temp[user][col] > temp[user][col1] :
                        count = count + 1
                        #print col,count
        ...
        while True:
            ...
            if self.remArms() > self.rank:
                ...
                if self.remExplore() > 0:
                    ...
                    for col in range(0,self.numActions):
                        if self.expl[col] != -1 and self.estR[user][col]== self.MIN and col in self.bestCol:
                            self.action = col
