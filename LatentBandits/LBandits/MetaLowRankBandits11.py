...
class MetaLowRankBandit(object):
    ...
    def select_Col(self, user_nature, action):
        ...
        #print self.prob_user[user_nature][action]
        xj = [0.0 for col in range(0,self.numActions)]
        xj[action] = (theReward/self.prob_user[user_nature][action])
        ...
        for col in range(0,self.numActions):
            ...
            self.weights_user[user_nature][col] = self.weights_user[user_nature][col] + (self.gamma*xj[col]/self.numActions)
        
        '''
        max_weight = max(self.weights_user[user_nature])     
        for col in range(0,self.numActions):
            self.prob_user[user_nature][col] = math.exp(self.weights_user[user_nature][col] - max_weight)
            
            
        sum1 = sum(self.prob_user[user_nature])
        for col in range(0,self.numActions):
                    
            self.prob_user[user_nature][col]=(1.0-self.gamma)*(self.prob_user[user_nature][col]/(sum1)) + (self.gamma/self.numActions)
        '''                  
        '''
        for col in range(self.numActions):
            self.theta[user_nature][col]=random.betavariate((self.payoffSums[user_nature][col]+1.0),(self.numPlays[user_nature][col]-self.payoffSums[user_nature][col]+1.0))
        '''
    ...
    def user_bandit_EXP3(self, user):
        ...
        #print "user EXP3"
        max_weight = max(self.weights_user[user])
        for col in range(0,self.numActions):
            self.prob_user[user][col] = math.exp(self.weights_user[user][col] - max_weight)
            '''
            if col in self.bestActionSet:
                self.prob_user[user][col] = math.exp(self.weights_user[user][col] - max_weight)
            else:
                self.prob_user[user][col] = 0.0
            '''
        sum1 = sum(self.prob_user[user])
        for col in range(0,self.numActions):
            
            self.prob_user[user][col]=(1.0-self.gamma)*(self.prob_user[user][col]/(sum1)) + (self.gamma/self.numActions)
            ''' 
            if col in self.bestActionSet:
                self.prob_user[user][col]=(1.0-self.gamma)*(self.prob_user[user][col]/(sum1)) + (self.gamma/self.numActions)
            else:
        ...
        sortedProb = list(numpy.sort(self.prob_user[user]))
        action1 = 0
        num = random.uniform(0,1)
        cum = 0.0
        for b in range(0,self.numActions):
            ...
            if num <= cum:
                ...
                for c in range(0,self.numActions):
                    if sortedProb[b] == self.prob_user[user][c]:
        ...
        #print "p: " + str(self.prob)
        #print "bA: " + str(self.bestActionSet)
        if isnan(sum(self.prob_user[user])) == True:
            print self.R
            print self.a
            print self.bestActionSet
            print self.prob[user]
            print self.weights[user]
    ...
    def MetaLowRankBandit(self, users, numActions, rank, readfile, writefile):
        ...
        self.weights_user = [[1.0 for i in range(0, self.numActions)] for j in range (0,self.users)]
        self.prob_user = [[0.0 for i in range(0, self.numActions)] for j in range (0,self.users)]
        ...
        for user in range(0,self.users):
            for col in range(0,self.numActions):
                self.prob_user[user][col] = (1.0-self.gamma)*(self.weights_user[user][col]/(sum(self.weights_user[user]))) + (self.gamma/self.numActions)
        ...
        print self.gamma/self.numActions
        ...
        while True:
            ...
            while True:
                ...
                self.action = self.user_bandit_EXP3(self.user_nature)
            ...
            if count >= self.rank:
                ...
                if len(self.aS) >= self.rank:
                    #if abs(self.prob_user[self.user_nature][self.bestActionSet[0]] - self.prob_user[self.user_nature][self.bestActionSet[1]]) < (self.gamma/self.numActions):
            ...
            if self.t % 1000 == 0:
                ...
                print self.prob_user[self.user_nature][self.bestActionSet[0]], self.prob_user[self.user_nature][self.bestActionSet[1]]
        ...
        f = open(writefile + 'testRegretMetaEXP0RR1.txt', 'a')
...
if __name__ == "__main__":
    ...
    for turn in range(0,1):
        ...
        f = open(writefile +'testMetaEXP0RR1.txt', 'a')
