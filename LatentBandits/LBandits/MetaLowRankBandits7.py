...
class MetaLowRankBandit(object):
                    ...
                    for c in range(0,self.numActions):
                        if sortedProb[b] == self.prob[bandit, c]:
                            ...
                            if action1 not in (self.bestActionSet): 
                                ...
                                #Set the weight of that action to 0 so that it is not chosen by others
                                
                                for bandit1 in range(0,self.rank):
                                    if action1 in self.bestActionSet and bandit != bandit1:
                                        #self.prob[bandit1][action1] = 0  
                                        self.weights[bandit1, action1] = self.gamma/self.numActions
    ...
    def metaBanditEXP3_1(self):
        ...
        for bandit in range(0,self.rank):
            ...
            #cum_sum = numpy.cumsum(numpy.sort(self.prob[bandit]))
            cum_sum = numpy.cumsum(self.prob[bandit])
            #print self.prob[bandit]
            #print cum_sum
            
            #cum_sum = numpy.cumsum(numpy.array([0.1, 0.4, 0.3, 0.2]))
            num = random.uniform(0,1)
            action = numpy.flatnonzero(num < cum_sum)[0]
            ...
            if action not in (self.bestActionSet): 
                ...
                for bandit1 in range(0,self.rank):
                    if action in self.bestActionSet and bandit != bandit1:
                        #self.prob[bandit1][action1] = 0  
                        self.weights[bandit1, action] = 0
    ...
    def updateWeight(self):
        ...
        xj = numpy.zeros(self.numActions)
    ...
    def MetaLowRankBandit(self, users, numActions, rank, readfile, writefile):
        ...
        self.payoffSums = numpy.zeros((self.users,self.numActions))
        #print self.payoffSums
        self.numPlays = numpy.zeros((self.users,self.numActions))
        self.estR = numpy.zeros((self.users,self.numActions))
        
        self.ucbs = numpy.zeros((self.users,self.numActions))
        self.lcbs = numpy.zeros((self.users,self.numActions))
        ...
        self.arm_reward = numpy.zeros((self.users,self.numActions))
        self.bestAction = numpy.zeros(self.users)
        self.bestSet = numpy.zeros(self.users)

        self.means = numpy.zeros((self.users,self.numActions))
        ...
        self.weights = numpy.ones((self.rank,self.numActions))
        self.prob = numpy.zeros((self.rank,self.numActions,))
