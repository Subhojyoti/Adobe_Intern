                            ...
                            if action1 not in (self.bestActionSet): 
                                ...
                                for bandit1 in range(0,self.rank):
                                    if action1 in self.bestActionSet and bandit!=bandit1:
                                        ...
                                        for col in range(0,self.numActions):
                                            self.weights[bandit1][col] = self.weights[bandit1][col]/sum1
    ...
    def changeWeight(self):
        ...
        for d in self.aS:
            ...
            for col in range(0,len(self.a)):
                if self.a[col] == d:
                    max_val.append(self.R[col])
                    max_val = self.R
                    max_index += 1
            
            #max_take.append(max_val[random.randint(0,len(max_val) - 1)])
            max_take.append(sum(max_val)/max_index)
        '''    
        ...
        print max_take
        '''
        #for d in range(0,len(self.aS)):
        
        for d in self.aS:
            if d == self.bestActionSet[0]:
                
                #print self.R[d],self.a[d]
                #r1 = self.R[max_take[0]]
                #r1 = abs(max_take[1] - max_take[0])
                #r1 = max_take[0]
                #r1 = max(max_take[1], max_take[0]) - max_take[1]
                r1 = self.R[0]
                #r1 = max(self.R)
                
                xj = [0.0 for col in range(0,self.numActions)]
                xj[d] = r1/self.prob[0][d]
                for col in range(0,self.numActions):
            ...
            elif d == self.bestActionSet[1]:
                #max_index = max(range(0,len(self.R)), key=lambda col: self.R[col])
                
                #r2 = max(self.R) - self.R[min_index]
                #r2 = self.R[min_index]
                #r2 = (self.R[max_take[0]]+self.R[max_take[1]]) - self.R[max_take[0]]
                #r2 = abs(max_take[1] - max_take[0])
                #r2 = max(max_take[1], max_take[0])
                #r2 = max(max_take[1], max_take[0]) - max_take[0]
                r2 = self.R[1]
                #r2 = max_take[1]
                #r2 = self.R[R1[0]] - self.R[R1[1]]
                xj = [0.0 for col in range(0,self.numActions)]
                xj[d] = r2/self.prob[1][d]
                for col in range(0,self.numActions):
        ...
        while True:
            ...
            if len(self.aS) >= self.rank:
                ...
                while True:
                    ...
                    #self.bestActionSet = sorted(self.bestActionSet)
                    set1 = sets.Set(self.bestActionSet)
                    if len(set1) >= self.rank:
                        break
