...
class GLBUCB(object):
    ...
    def upperBound(self, user, col):
        ...
        return math.sqrt(math.log((self.psi*self.numRounds*self.epsilon*self.epsilon))/(16.0*self.numPlays[user][col]))
                ...
                #Compare against other column max row by row in the same equivalence class
                    ...
                    for row in range(self.equiValence[c],self.equiValence[c+1]):  
                        ...
                        if self.B[col]!=-1 and self.numPlays[row][col]!= 0.0:
                            ...
                            if self.estR[row][col] + self.upperBound(row, col) < self.estR[row][index] - self.upperBound(row, col):
    ...
    def GLBUCB(self, users, numActions, rank, readfile):
        ...
        self.psi = (self.numRounds/(self.numActions*self.numActions))
        #self.psi = 1.0
        self.epsilon = 1.0
        ...
        self.n0 = 5
        self.n1 = 5
        self.Nmx = self.users*(len(self.bestCol[0])+1)*self.n0
        self.Nm =  self.rank*self.users*self.nm + self.Nmx 
        #self.Nm =  0.0 + self.Nmx 
        print self.Nm, self.nm, self.Nmx
        ...
        while True:
            ...
            if self.remArms() > self.rank:
                ...
                #Explore the column if not explored
                    ...
                    self.action = self.choose_Col_RR2(self.user_nature)
                    self.select_Col()
                    ...
                    self.Nm =  self.Nmx + self.rank*self.users*self.nm 
