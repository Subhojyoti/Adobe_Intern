...
class GLBUCB(object):
    ...
    def fullExplore(self):
        ...
        for j in range(0,self.numActions):
            count=0
            if self.expl[j]!=-1 and self.B[j]!=-1:
    ...
    def upperBound(self):
        ...
        # Noisy
        return math.sqrt(math.log((self.psi*self.numRounds*self.epsilon*self.epsilon))/(8.0*self.nm))
    ...
    def find_best_cols1(self):
        ...
        for user in range(0,self.users):
            for col in range(0,self.numActions):
                ...
                for col1 in range(0,self.numActions):
                    if self.B[col]!= -1 and self.B[col1]!=-1 and col!= col1 and self.estR[user][col]!= self.MIN and self.estR[user][col1]!=self.MIN and self.estR[user][col] - self.upperBound() >= self.estR[user][col1] + self.upperBound():
                    #if self.B[col]!= -1 and self.B[col1]!=-1 and col!= col1 and self.estR[user][col] - self.upperBound() > self.estR[user][col1] + self.upperBound() :
                        count = count + 1
                        #print col,count
                
                if count >= self.numActions - 1:    
                    calc[user][col] = count
        ...
        print "Dominance: " + str(take) + str(self.upperBound())
        while True:
            ...
            if len(self.bestCol) >= 2:
                break
            
    def find_best_cols2(self):
        ...
        for user in range(0,self.users):
            ...
            index = max(range(0,self.numActions), key=lambda col: self.estR[col])  
            calc[user][index] += 1 
        ...
        print "Dominance: " + str(take) + str(self.nm)
        while True:
    ...
    def GLBUCB(self, users, numActions, rank, readfile):
        ...
        while True:
            ...
            if self.remArms() > self.rank:
                ...
                #Explore the column if not explored
                    ...
                    if self.remExplore() > 0:
                        if self.countRow <= self.explore:
                            ...
                            self.action = self.choose_Col_RR1(user)
                            self.expl[self.action] = -1 
                            self.countRow = 1
                    else:
                ...
                #Explore 2nd half   
                    ...
                    if self.flag == True:
                        ...
                        self.flag = False
                        print "Phase: " + str(self.m) + " Best Col: " +str(self.bestCol)
                        self.action = self.choose_Col_RR1(user)
                        print self.action, self.expl
                        self.expl[self.action] = -1 
                    ...
                    if self.remExplore() > 0:
                        ...
                        self.expl = [0 for i in range(0,self.numActions)]
                        for i in range(0,self.numActions):
                            if i not in self.bestCol:
                                self.expl[i] = -1
                        
                        #print self.expl, self.bestCol
                        self.action = self.choose_Col_RR1(user)
                        self.expl[self.action] = -1 
                        self.countRow = 1
                ...
                if self.fullExplore() > 0:
                    ...
                    for col in range(0,self.numActions):
                        if self.estR[user][col]== self.MIN and col in self.bestCol:
                            self.action = col
                            break
            ...
            if self.t % 100 == 0:
                print "At time: " + str(self.t), ", action: " +str(self.action), str(self.bestAction[user]) , ", regret:", str(regret)
        ...
        f = open('NewExpt/expt8/testRegretGLBUCB0RR2.txt', 'a')
...
if __name__ == "__main__":
    ...
    for turn in range(0,1):
        ...
        f = open('NewExpt/expt8/testGLBUCB0RR2.txt', 'a')
