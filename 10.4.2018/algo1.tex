...
\begin{algorithmic}[1]
\State {\bf Input:} Time horizon $T$, $Rank(\bar{R}) = d$.
\State {\bf Definition:} $U(i,j) = \sqrt{\dfrac{2\log T}{N_{i,j}}}$.
\For{$t=1,..,T$}	
\State Nature reveals user $i_t$.  \Comment{Nature chooses user}
\State Meta-EXP3 suggests $\B_t\in[L]$ items. $|\B_t| = d$
\If{$\left(\hat{R}(i_t,j) - U(i_t,j) \leq \hat{R}(i_t,j') + U(i_t,j')\right), \forall j,j' \in \B_t$}	 \Comment{UCB overlap, Exploration}
\State User Expert suggests each arm $j\in\B_t$ once to user $i_t$ and receive feedback.
