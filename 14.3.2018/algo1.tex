...
\EndFor
\State $Z_{\G_b, 0}\leftarrow$ Select $\gamma$ random columns for each equivalence class $\G_b\in\C, \forall b\in \left[1,\frac{K}{d}\right] $ and ensure $\bigcup\limits_{b=1}^{\frac{K}{d}}Z_{\G_b,0} = [L]$. %and put then in $Z_{\G_b, 0}$.
\For{$t=1,..,T$}	
